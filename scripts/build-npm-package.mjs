...
assertExists(nextStandaloneDir, "Next standalone build missing. Run `npm run build` first.");
assertExists(nextStaticDir, "Next static assets missing. Run `npm run build` first.");
...
const platformDashboardDir = path.join(platformPackageDir, "dashboard", "standalone");
const platformStaticDir = path.join(platformDashboardDir, ".next", "static");
...
fs.cpSync(nextStandaloneDir, platformDashboardDir, { recursive: true });
fs.mkdirSync(path.dirname(platformStaticDir), { recursive: true });
fs.cpSync(nextStaticDir, platformStaticDir, { recursive: true });
...
writeJson(path.join(basePackageDir, "package.json"), {
  name: "openproxy",
  version,
  description: "One-command launcher for the OpenProxy Rust server and packaged Next.js dashboard.",
  bin: {
    openproxy: "index.cjs",
  },
...
fs.writeFileSync(
  ...
  [
    "# openproxy",
    "",
    "Launcher package that boots the Rust backend on `4623` and the packaged Next.js sidecar on `4624`.",
    "",
...
console.log("OpenProxy launcher: Rust " + host + ":" + port + ", Next 127.0.0.1:" + dashboardPort);
...
function resolveRuntime() {
  ...
  const dashboardServer = path.join(packageRoot, "dashboard", "standalone", "server.js");
